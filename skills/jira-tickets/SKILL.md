---
name: jira-tickets
description: Create and update Jira tickets with action-verb summaries and ADF descriptions (user story, References, Acceptance Criteria). Use when creating, updating, or reformatting Jira issues, writing ticket descriptions, or submitting descriptions via the Jira Cloud REST API.
---
...
| `- [ ] Criterion` | `taskList` → `taskItem` attrs `state: "TODO"` (each needs a unique `localId`) |
...
MCP is fine for read-only operations (search, get issue) and non-description fields (summary, epic link, transitions).
